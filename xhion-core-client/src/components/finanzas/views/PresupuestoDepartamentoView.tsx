...
interface PresupuestoDepartamentoViewProps {
  departamentoId: string;
  departamentoNombre?: string;
}

export default function PresupuestoDepartamentoView({
  departamentoId,
  departamentoNombre,
}: PresupuestoDepartamentoViewProps) {
  const { presupuestoDepartamento, loading, obtenerPresupuestoDepartamento } = useFinanzasStore();
    ...
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {departamentoNombre ? `Departamento: ${departamentoNombre}` : 'Gestión del presupuesto'}
              </CardDescription>
            </div>
            {getEstadoBadge()}
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-lg font-semibold">
                {format(new Date(fechaInicio), 'dd/MM/yyyy', { locale: es })}
              </p>
            </div>
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground flex items-center gap-1">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Últimos movimientos del presupuesto</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {movimientos.length === 0 ? (
                  ...
                  movimientos.slice(0, 10).map((mov: any) => (
                    <TableRow key={mov.id}>
                      <TableCell>
      ...
      <RegistrarMovimientoModal
        ...
        onSuccess={() => obtenerPresupuestoDepartamento(departamentoId)}
      />
    </div>
  );
}
